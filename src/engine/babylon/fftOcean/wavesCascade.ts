...
import timeDependentSpectrumCS from './shaders/timeDependentSpectrum.wgsl?raw'
import wavesTexturesMergerCS from './shaders/wavesTexturesMerger.wgsl?raw'
import { FFT } from './fft'
...
export class WavesCascade {
  ...
  private _sampleTimeSpectrum = true
  private _timeSpectrumRange = { min: 0, max: 0 }
  private _timeDispatches = { succeeded: 0, failed: 0 }
  ...
  public get initialSpectrum() {
    return this._initialSpectrum.initialSpectrum
  }

  public get timeSpectrumRange() {
    return this._timeSpectrumRange
  }

  public get timeDispatches() {
    return this._timeDispatches
  }
  ...
  constructor(
    ...
    this._initialSpectrum = new InitialSpectrum(
      engine,
      rttDebug,
      debugFirstIndex,
      size,
    ...
    this._timeDependentSpectrum.setTexture(
      'H0',
      this._initialSpectrum.initialSpectrum,
      false,
    )
    this._timeDependentSpectrum.setTexture(
      'WavesData',
      this._initialSpectrum.wavesData,
      false,
    )
    this._timeDependentSpectrum.setUniformBuffer(
      'params',
      this._timeDependentSpectrumParams,
    )
  ...
  public calculateInitials(
    ...
    this._initialSpectrum.generate(
      wavesSettings,
      lengthScale,
      cutoffLow,
      cutoffHigh,
  ...
  public calculateWavesAtTime(time: number): void {
    ...
    const dispatched = ComputeHelper.Dispatch(
      this._timeDependentSpectrum,
      this._size,
      this._size,
      1,
    )
    if (dispatched) this._timeDispatches.succeeded += 1
    else this._timeDispatches.failed += 1

    if (this._sampleTimeSpectrum) {
      this._sampleTimeSpectrum = false
      this._DyDxz
        ...
        ?.then((buffer: ArrayBufferView) => {
          ...
          if (Number.isFinite(min) && Number.isFinite(max)) {
            this._timeSpectrumRange = { min, max }
          }
        })
    }
    ...
    this._texturesMergerParams.updateFloat(
      'DeltaTime',
      (deltaTime * this.foamDecay) / 0.5,
    )
    ...
    this._engine.generateMipmaps(
      this._pingPongTurbulence
        ? this._turbulence2.getInternalTexture()!
        : this._turbulence.getInternalTexture()!,
    )
  }
