...
export class Buoyancy {
  ...
  public setWaterHeightMap(
    map: BABYLON.Nullable<Uint16Array>,
    lengthScale: number,
  ): void {
  ...
  private _updateMesh(meshBuoyancy: MeshBuoyancy): void {
    ...
    const y = this.getWaterHeight(tmp) * this.heightScale

    mesh.position.y = y + yOffset + this.yOffset
    ...
    if (frame.v2 && frame.v3) {
      ...
      let xa =
        Math.acos(
          BABYLON.Scalar.Clamp(BABYLON.Vector3.Dot(forwardU, forward), 0, 1),
        ) *
        this._attenuation *
        this.pitchScale
      ...
      let za =
        Math.acos(
          BABYLON.Scalar.Clamp(BABYLON.Vector3.Dot(rightU, right), 0, 1),
        ) *
        this._attenuation *
        this.rollScale
  ...
  private _getDisplacement(
    ...
    if (this._displacementMap) {
      result.x = BABYLON.TextureTools.FromHalfFloat(
        this._displacementMap[z * this._size * 4 + x * 4 + 0],
      )
      result.y = BABYLON.TextureTools.FromHalfFloat(
        this._displacementMap[z * this._size * 4 + x * 4 + 1],
      )
      result.z = BABYLON.TextureTools.FromHalfFloat(
        this._displacementMap[z * this._size * 4 + x * 4 + 2],
      )
    }
  }
}
