...
export class ComputeHelper {
  ...
  static CopyTexture(
    source: BABYLON.BaseTexture,
    dest: BABYLON.BaseTexture,
    engine_?: BABYLON.Engine,
  ): void {
    const numChannels =
      source.getInternalTexture()!.format === BABYLON.Constants.TEXTUREFORMAT_RG
        ? 2
        : 4
    if (
      ...
      const cs1 = new BABYLON.ComputeShader(
        ...
        engine,
        {
          computeSource:
            numChannels === 4
              ? ComputeHelper._copyTexture4ComputeShader
              : ComputeHelper._copyTexture2ComputeShader,
        },
        {
          bindingsMapping: {
    ...
    const cs =
      numChannels === 4
        ? ComputeHelper._copyTexture4CS
        : ComputeHelper._copyTexture2CS
    const params =
      numChannels === 4
        ? ComputeHelper._copyTexture4Params
        : ComputeHelper._copyTexture2Params
  ...
  static CopyBufferToTexture(
    source: BABYLON.StorageBuffer,
    dest: BABYLON.BaseTexture,
  ): void {
  ...
  static CopyTextureToBuffer(
    source: BABYLON.BaseTexture,
    dest: BABYLON.StorageBuffer,
  ): void {
  ...
  static Dispatch(
    ...
    return cs.dispatch(numGroupsX, numGroupsY, numGroupsZ)
  }
}
