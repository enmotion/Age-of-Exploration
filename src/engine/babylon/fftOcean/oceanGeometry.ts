...
export class OceanGeometry {
  ...
  constructor(
    ...
    this._trimRotations = [
      ...
      BABYLON.Quaternion.RotationAxis(
        BABYLON.Vector3.UpReadOnly,
        BABYLON.Angle.FromDegrees(270).radians(),
      ),
  ...
  public set seaLevel(value: number) {
    this._root.position.y = value
  }

  public get seaLevel(): number {
    return this._root.position.y
  }
  ...
  private _updateMaterials(): void {
    ...
    this._center.material = this._getMaterial(
      this.noMaterialLod ? 0 : this.clipLevels - activeLevels - 1,
    )

    for (let i = 0; i < this._rings.length; i++) {
      this._rings[i].material = this._getMaterial(
        this.noMaterialLod ? 0 : this.clipLevels - activeLevels - i,
      )
      this._trims[i].material = this._getMaterial(
        this.noMaterialLod ? 0 : this.clipLevels - activeLevels - i,
      )
    }

    if (this.useSkirt) {
      this._skirt.material = this.noMaterialLod
        ? this._materials[0]
        : this._materials[2]
    }
  }
  ...
  private _updatePositions(): void {
    ...
    for (let i = 0; i < this.clipLevels; i++) {
      ...
      this._trims[i].rotationQuaternion!.copyFrom(
        this._trimRotations[shiftX + 2 * shiftZ],
      )
    ...
    if (this.useSkirt) {
      ...
      this._skirt.position
        ...
        .addInPlaceFromFloats(
          -scale * (this.skirtSize + 0.5 - 0.5 / k),
          0,
          -scale * (this.skirtSize + 0.5 - 0.5 / k),
        )
  ...
  private get _activeLodLevels(): number {
    return (
      ...
      BABYLON.Scalar.Clamp(
        Math.floor(
          Math.log2(
            (1.7 * Math.abs(this._camera.position.y) + 1) / this.lengthScale,
          ),
        ),
        0,
        this.clipLevels,
  ...
  private _clipLevelScale(level: number, activeLevels: number): number {
    return (
      (this.lengthScale / this._gridSize) *
      Math.pow(2, this.clipLevels - activeLevels + level + 1)
    )
  }

  private _offsetFromCenter(
    level: number,
    activeLevels: number,
    ...
    const v =
      (((((1 << this.clipLevels) +
        OceanGeometry._GeometricProgressionSum(
          2,
          2,
          this.clipLevels - activeLevels + level + 1,
          this.clipLevels - 1,
        )) *
        this.lengthScale) /
        k) *
  ...
  private static _GeometricProgressionSum(
    b0: number,
    q: number,
    n1: number,
    n2: number,
  ...
  private _createSkirtMesh(k: number, outerBorderScale: number): BABYLON.Mesh {
    ...
    const cornerQuadScale = new BABYLON.Vector3(
      outerBorderScale,
      1,
      outerBorderScale,
    )
  ...
  private _createRingMesh(k: number, lengthScale: number): BABYLON.Mesh {
    const m1 = this._createPlaneMesh(
      2 * k,
      (k - 1) >> 1,
      lengthScale,
      Seams.Bottom | Seams.Right | Seams.Left,
    )

    const m2 = this._createPlaneMesh(
      2 * k,
      (k - 1) >> 1,
      lengthScale,
      Seams.Top | Seams.Right | Seams.Left,
    )
    m2.position.set(0, 0, (k + 1 + ((k - 1) >> 1)) * lengthScale)

    const m3 = this._createPlaneMesh(
      (k - 1) >> 1,
      k + 1,
      lengthScale,
      Seams.Left,
    ...
    const m4 = this._createPlaneMesh(
      (k - 1) >> 1,
      k + 1,
      lengthScale,
      Seams.Right,
  ...
  private _createPlaneMesh(
    ...
    for (let i = 0; i < height + 1; ++i) {
      for (let j = 0; j < width + 1; ++j) {
        ...
        if (
          (i === 0 && seams & Seams.Bottom) ||
          (i === height && seams & Seams.Top)
        ) {
          x = x & ~1
        }
        if (
          (j === 0 && seams & Seams.Left) ||
          (j === width && seams & Seams.Right)
        ) {
          z = z & ~1
        }
